...
M.shift = function(direction)
    ...
    if direction == M.DIRECTION_LEFT and winframe.x ~= frame.x then
        ...
        return
    elseif
        direction == M.DIRECTION_RIGHT
        and (winframe.x + winframe.w) ~= frame.w
    then
...
M.shift_v = function(direction)
    ...
    -- move to top/bottom
    if direction == M.DIRECTION_UP and winframe.y ~= frame.y then
        winframe.y = frame.y
        win:setFrame(winframe)
    elseif
        direction == M.DIRECTION_DOWN
        and (winframe.y + winframe.h) ~= frame.y
    then
